...
def classify_email(email: dict[str, Any]) -> dict[str, Any]:
    """Return a category, explainable rule, and calibrated rule confidence."""
    ...
    if sender_domain in SPAM_DOMAINS:
        return {"category": SPAM, "rule": f"known spam sender domain: {sender_domain}", "confidence": 0.99}
    if any(phrase in combined_lower for phrase in SPAM_PHRASES):
        return {"category": SPAM, "rule": "explicit spam or phishing phrase", "confidence": 0.98}

    for pattern in INVOICE_SUBJECT_PATTERNS:
        if pattern.search(clean_subject):
            return {"category": INVOICE_QUERY, "rule": f"invoice subject pattern: {pattern.pattern}", "confidence": 0.96}

    if re.match(r"^(?:request\s+si|cust\s+si|si\s+needed)(?:\b|\s*[_-])", subject_lower):
        return {"category": SI_REQUEST, "rule": "explicit SI request subject", "confidence": 0.97}
    if re.match(r"^si\s+-", subject_lower):
        return {"category": SI_REQUEST, "rule": "shipping instruction subject", "confidence": 0.94}

    if re.match(r"^(?:to\s+confirm\s+docs|request\s+bl\s+draft|draft\s+bl)(?:\b|\s*[_-])", subject_lower):
        return {"category": BL_COMPARISON, "rule": "explicit BL checking subject", "confidence": 0.97}
    if re.match(r"^(?:aie|afemy|afrt|afptme)\s+-", subject_lower):
        return {"category": BL_COMPARISON, "rule": "coded BL comparison subject", "confidence": 0.93}

    return {"category": GENERAL, "rule": "no specialized category rule matched", "confidence": 0.82}
